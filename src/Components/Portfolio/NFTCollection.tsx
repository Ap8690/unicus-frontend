...
import { getUserWallet, connectWallet, selectNetwork, decodeParams } from "../../Utilities/Util";
...
const NFTCollection = ({ item, transID }: any) => {
  ...
  const createSaleForNFT = async (e: any) => {
    console.log(item);
    console.log(tronWeb.address.fromHex())
    e.preventDefault();
    ...
    try {
      ...
      if (item.uploadedBy === "61e559cb515235e5d16373fe") {
        ...
        await connectWallet(item.chain)
          .then(async () => {
            ...
            console.log(marketPlace, marketAddress,"these");
            ...
            if (networkID === tronChain) {
              ...
              const saleId = await decodeParams(['uint256'],"0x"+success?.log[2]?.topics[1],false)
              console.log(success,saleId)
              console.log(tronWeb.address.fromHex(saleId[0]))

              if (success) {
                obj.auctionId = tronWeb.address.fromHex(saleId[0]);
                obj.auctionHash = success?.id
              } else {
