...
const NFTCollection = ({ item, transID }: any) => {
                            ...
                            await axios
                                .post(
                                    `${backendUrl}/auction/sell`,
                                    {
                                        ...
                                        sellerInfo: userInfo.username,
                                        auctionId:
                                            res.events.saleCreated.returnValues
                                                .itemId,
                                        startBid: SellPrice,
                                        auctionType: "Sale",
    ...
    // end auction integration
    async function endAuction(e: any) {
        e.preventDefault();
        try {
            setNftLoading(true);
            setLoadingMessage("Please Connect to your wallet...");
            const endDate = new Date(item.auctionTimer)
            if (nowDate < endDate) {
                setNftLoading(false);
                ...
                if (false) {
                    ...
                    connectWallet()
                        .then(async () => {
                            ...
                            setLoadingMessage(
                                "Confirm in Metamask popup to End your Auction..."
                            );
                            const res = await auction.methods
