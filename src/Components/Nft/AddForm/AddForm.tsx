...
import { avalancheChain, backendUrl, tronChain } from "../../../config";
// img
          ...
          try {
            axios
              ...
              .then(async (mintAPI: any) => {
                ...
                formData.append(
                  "tokenId",
                  mintAPI.data.events.Minted.returnValues._NftId
                ); //returnValues NFTId
                formData.append("uploadedBy", "61e559cb515235e5d16373fe"); // Admin ID if metamask then userId
                ...
                await axios
                  .post(`${backendUrl}/nft/create`, newObject, newaxiosConfig)
                  .then((res: any) => {
  ...
  const cryptoPayment = async (e: any) => {
    ...
    connectWallet(networkID)
      .then(async () => {
        ...
        var contractAddress;
        console.log(typeof networkID, bscChain)
        if (networkID === bscChain) {
          contractAddress = createNFTAddressB;
        } else if (networkID === ethChain) {
          contractAddress = createNFTAddressE;
        } else if (networkID === polygonChain) {
          contractAddress = createNFTAddressP;
        } else if (networkID === tronChain) {
          contractAddress = createNFTAddressT;
        } else if (networkID === avalancheChain) {
          console.log(avalancheChain,networkID,"done")
          contractAddress = createNFTAddressA;
        }
        ...
        try {
          ...
          if (networkID === tronChain) {
            ...
            const tokenId = tronWeb.address.fromHex(await decodeParams(['uint256'],"0x"+success?.log[0]?.topics[3],false))
          ...
          if(tranIsSuccess){
            ...
            if (
              imageSrc[0].type == "video/mp4" ||
              imageSrc[0].type == "image/gif"
            ) {
              cloudinaryFormData.append(
                "upload_preset",
                `Unicus___${networkID}___video`
        ...
        <Container>
          <Row>
            <Col xs={12} sm={12} md={12} lg={4} xl={4} className="mb-3">
              <div className="add_nft__left" style={{ marginTop: "2rem" }}>
                <h5>Preview NFT</h5>
                <div className="preview_card">
                  <div className="prevImg">
                    {imageSrc.length > 0 &&
                    imageSrc[0] &&
                    imageSrc[0].path.split(".").pop() == "mp4" ? (
                      ...
                      imageSrc.length > 0 && (
                        <Image src={URL.createObjectURL(imageSrc[0])} alt="" />
                      )
                    )}
                  </div>
                  <h6>
          ...
          <div className="buy__cpt__modal">
            ...
            <div className="success__body create_nft_modal">
              ...
              >
                <div
                  className="wallet nftmodal newChange"
                  style={{ width: "100%", padding: "29.5px" }}
                >
                  <h5>Pay Using Stripe</h5>
                </div>
              </StripeCheckout>
            </div>
          </div>
