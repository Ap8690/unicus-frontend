...
import {mintNFTB, mintNFTP, mintNFTE, bscChain, ethChain, polygonChain} from '../../config'
...
const NFTById = (props: any) => {
 ...
 : "none"}/${location.pathname.split("/")[2]}`
                ...
                .then(async (res: any) => {
                    ...
                    if(res.data.auction) {
                        ...
                        setnowDate(new Date())
                        setdateArray(new Date(res.data.auction.auctionTimer).toString().split(' '))
                        setdateHours(new Date(res.data.auction.auctionTimer).toString().split(' ')[4].split(':'))
                        console.log(new Date(res.data.auction.auctionTimer).toString().split('(')[1].split(')')[0].split(' '))
                        var timeZone = ''
            ...
            await axios
                ...
                .then(async (res: any) => {
                    ...
                    setnowDate(new Date())
                    setdateArray(new Date(res.data[0].auctionTimer).toString().split(' '))
                    setdateHours(new Date(res.data[0].auctionTimer).toString().split(' ')[4].split(':'))
                    console.log(new Date(res.data[0].auctionTimer).toString().split('(')[1].split(')')[0].split(' '))
                    var timeZone = ''
 ...
 : "none"}/${location.pathname.split("/")[2]}`)
    ...
    useEffect(() => {
        ...
        if (paymentMode === 'MetaMaskPlaceBid' || paymentMode === 'StripePlaceBid') {
            placeBid()
        }
    }, [paymentMode])
                        ...
                        >
                            <div className='nft__image'>
                                <a
                                    ...
                                    target='_blank'
                                >
                                    {nftInfo && (nftInfo.cloudinaryUrl && nftInfo.cloudinaryUrl.split('.').pop() == "mp4") ? <video style={{width: '100%', maxHeight: '100%'}} autoPlay loop>
                                        <source src={nftInfo && nftInfo.cloudinaryUrl} type="video/mp4" />
                                    </video> : <img
                        ...
                        >
                            <div className='nft__right'>
                                <div className='header'>
                                    ...
                                    <p>
                                        Owned By{' '}
                                        {auctionInfo.auctionStatus == 3 ? <NavLink
                                            to={
                                                nftInfo &&
                                    ...
                                    <p>
                                        {((nftInfo.nftStatus !== 1) &&
                                            (auctionInfo.auctionType !==
                                                'Sale')) && ((endDate - nowDate) > 0) ?
                                            `Auction Ends ${dateArray[1]} ${dateArray[2]}, ${dateArray[3]} at ${dateHours[0] > 12 ? dateHours[0] - 12 : dateHours[0]}:${dateHours[1]}${dateHours[0] > 12 ? 'pm' : 'am'} ${dateZone}` : (auctionInfo.auctionStatus ===
                                                3 &&
                                                (userInfo &&
                                                    userInfo._id) ===
                                                    (auctionInfo &&
                                                        auctionInfo.sellerId) ? 'Auction Ended' : (auctionInfo.auctionStatus === 4 && 'Auction Canceled'))}
                                    </p>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    {!(userInfo && userInfo._id) ? (
                                        ...
                                        <>
                                            ...
                                            auctionInfo && auctionInfo.auctionType ===
                                                'Sale' ? (
                                                <div className='sale_buttons'>
                                              ...
                                              auctionInfo.auctionType ===
                                                  'Auction' ? (
                                                <div className='sale_buttons' style={{justifyContent: 'flex-start'}}>
                                                    {(done ||
                                                        !(
                                                            (userInfo &&
                                                                userInfo._id) ===
                    ...
                    <div className="upperGrid">
                      ...
                      <div className="accordionGrid">
                        ...
                        <div className='nft__normal nft__right nft__normal'>
                            <Accordion className='accordion' expanded={expanded === 'panel'} onChange={handleChangeExpanded('panel')}>
                                <AccordionSummary
                                    expandIcon={<ExpandMoreIcon />}
                                    aria-controls='panel1a-content'
                                    id='panel1a-header'
                                ...
                                >
                                    ...
                                    <div className='accordionGrid pl-15' style={{paddingLeft: '15px', alignItems: 'center'}}>
                                        <h6>Event</h6>
                                        <h6>Price</h6>
                                        <h6>From</h6>
                                        <h6>To</h6>
                                    ...
                                    <div className='upperGrid'>
                                        {nftStates &&
                                            nftStates.map((nftState: any) => {
                                                return (
                                                    <div className='accordionGrid' style={{paddingLeft: '15px', alignItems: 'center'}}>
                                                        <h6 className='flex' style={{justifyContent: 'left'}}>
                                                            {nftState.state ===
                                                            'Minted' ? (
                                                                <BsCartPlus />
                                                            ) : nftState.state ===
